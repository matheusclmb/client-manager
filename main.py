...
connector = sqlite3.connect("CLIENTManagement.db")
...
def add_record():
    ...
    if not name or not email or not contact or not gender or not DOB or not stream:
        ...
        try:
            connector.execute(
                "INSERT INTO CLIENT_MANAGEMENT (NAME, EMAIL, PHONE_NO, GENDER, DOB, STREAM) VALUES (?,?,?,?,?,?)",
...
def view_record():
    ...
    date = datetime.date(
        int(selection[5][:4]), int(selection[5][5:7]), int(selection[5][8:])
    )
...
main.title("CLIENT MANAGER @matheusclmb")
main.geometry("1300x600")
main.resizable(0, 0)

# Creating the background and foreground color variables
lf_bg = "#005f73"  # bg color for the left_frame
cf_bg = "#0a9396"  # bg color for the center_frame
...
Label(main, text="CLIENT MANAGER", font=headlabelfont, bg="#94d2bd").pack(
    side=TOP, fill=X
)
...
Label(left_frame, text="Contact Number", font=labelfont, bg=lf_bg).place(
    relx=0.175, rely=0.18
)
Label(left_frame, text="Email Address", font=labelfont, bg=lf_bg).place(
    relx=0.2, rely=0.31
)
Label(left_frame, text="Gender", font=labelfont, bg=lf_bg).place(relx=0.3, rely=0.44)
Label(left_frame, text="Date of Birth (DOB)", font=labelfont, bg=lf_bg).place(
    relx=0.1, rely=0.57
)
Label(left_frame, text="Address", font=labelfont, bg=lf_bg).place(relx=0.3, rely=0.7)

Entry(left_frame, width=19, textvariable=name_strvar, font=entryfont).place(
    x=20, rely=0.1
)
...
Button(
    ...
    command=add_record,
    width=18,
).place(relx=0.025, rely=0.85)

# Placing components in the center frame
Button(
    center_frame, text="Delete Client", font=labelfont, command=remove_record, width=15
).place(relx=0.1, rely=0.25)
Button(
    center_frame, text="View Client", font=labelfont, command=view_record, width=15
).place(relx=0.1, rely=0.35)
Button(
    center_frame, text="Reset Fields", font=labelfont, command=reset_fields, width=15
).place(relx=0.1, rely=0.45)
Button(
    center_frame, text="Delete database", font=labelfont, command=reset_form, width=15
...
tree.heading("Stream", text="Adress", anchor=CENTER)
